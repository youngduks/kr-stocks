...
const FILTERS = {
  ...
  hyundai: [
    ...
    "현대자동차","현대차","Hyundai Motor","Hyundai Motors",
    "기아차","기아자동차","Kia Motors","Kia Corp",
    "현대모비스","모비스","Hyundai Mobis",
    "현대차그룹","현대자동차그룹","HMG",
    // 인물 (회장/사장단)
    "정의선","정몽구","장재훈","송호성","호세 무뇨스",
    // 차종 (현대·기아·제네시스 — 다른 회사와 안 겹침)
    "아이오닉","Ioniq","IONIQ","제네시스","Genesis",
    "그랜저","팰리세이드","쏘나타","아반떼","캐스퍼","코나","투싼","스타리아","싼타페",
    "EV6","EV9","니로","K5","K8","카니발","스포티지","쏘렌토",
    // 현대차그룹 신사업 (specific 키워드만 — 휴머노이드/로보틱스 단독 제거)
...
// ─────────────────────────────────────────────────────────────
// Sentiment 분류 — 호재 / 악재 / 중립 (5/26 형님 요청, title 키워드 기반)
// ─────────────────────────────────────────────────────────────

const SENTIMENT_POSITIVE = [
  // 실적 호재
  "역대 최대","역대 최고","사상 최대","사상 최고","최고치","최대 실적",
  "어닝 서프라이즈","어닝서프라이즈","어닝 호조","실적 호조","실적 개선","실적 회복",
  "흑자 전환","흑자전환","적자 탈출","적자탈출",
  // 시장 호재
  "신고가","최고가","최고가 경신","52주 신고가","사상 최고가",
  "급등","상승세","랠리","돌파","폭등","훨훨",
  "사상 첫","역대 첫","최초",
  // 비즈니스 호재
  "수주","대규모 수주","수주 잭팟","수주 잭팟","수주 호조","대형 계약",
  "수출 호조","수출 증가","수출 신기록","수출 1위",
  "흥행","대박","호평","호조","상승 견인",
  // 신사업 / 신제품
  "신제품","공급 계약","대형 수주",
  // M&A / IPO
  "인수 성공","상장 흥행","IPO 흥행","흥행 성공",
  // 평가
  "긍정적","낙관","기대감","목표가 상향","투자의견 상향","매수 추천",
];

const SENTIMENT_NEGATIVE = [
  // 실적 악재
  "어닝 쇼크","어닝쇼크","적자 전환","적자전환","적자",
  "손실","감익","역성장","실적 부진","실적 악화",
  // 시장 악재
  "급락","폭락","하락세","하락 마감","쇼크","최저가","52주 신저가",
  "낙폭","하향","약세",
  // 비즈니스 악재
  "리콜","결함","화재","폭발","집단소송","소송","피소","파산",
  // 규제 악재
  "조사 착수","압수수색","기소","벌금","과징금","제재","독점 의혹","담합 의혹",
  "공정위 조사","검찰 수사",
  // 노사 악재
  "파업","총파업","파업 결의","노사 갈등","노조 반발","임금 협상 결렬",
  // 평가 악재
  "악재","우려","위기","경고","비관","부정적",
  "목표가 인하","투자의견 하향","매도 추천","하향 조정",
];

function classifySentiment(title) {
  if (!title) return "neutral";
  // NEGATIVE 우선 (악재가 더 중요한 정보)
  for (const k of SENTIMENT_NEGATIVE) {
    if (title.includes(k)) return "negative";
  }
  for (const k of SENTIMENT_POSITIVE) {
    if (title.includes(k)) return "positive";
  }
  return "neutral";
}
